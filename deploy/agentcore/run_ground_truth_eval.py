...
# --judge-model if your account's Bedrock access differs.
DEFAULT_JUDGE_MODEL_ID = "eu.anthropic.claude-sonnet-4-5-20250929-v1:0"

_JUDGE_PROMPT = """You are grading whether an AI system's answer to a question \
...
def _judge(bedrock_client, model_id: str, query: str, expected_answer: str, actual_answer: str) -> tuple[str, str]:
    prompt = _JUDGE_PROMPT.format(query=query, expected_answer=expected_answer, actual_answer=actual_answer)
...
def _evaluate_row(row: GroundTruthRow, mcp_client, bedrock_client, judge_model_id: str, session_id: str) -> EvalResult:
    ...
    try:
        ...
        result.verdict, result.rationale = _judge(
