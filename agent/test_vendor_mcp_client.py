...
connect_linkup() themselves need a real network round trip, not
unit-testable here). Run from within this directory, same as
test_direct_agent_merge.py:
...
def test_connect_vendor_dispatches_by_source_id(monkeypatch):
    ...
    monkeypatch.setattr(vmc, "connect_linkup", lambda: ("linkup-client", ["linkup-tool"]))
    ...
    monkeypatch.setitem(vmc.VENDOR_CONNECTORS, "linkup", vmc.connect_linkup)

    assert vmc.connect_vendor("cala") == ("cala-client", ["cala-tool"])
    assert vmc.connect_vendor("sayari") == ("sayari-client", ["sayari-tool"])
    assert vmc.connect_vendor("linkup") == ("linkup-client", ["linkup-tool"])
